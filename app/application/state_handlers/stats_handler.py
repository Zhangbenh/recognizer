...
class StatsHandler(BaseStateHandler):
	...
	def on_enter(self, ctx: StateContext):
		try:
			snapshot = self._statistics_query_service.snapshot_for_region(ctx.selected_region_id or "")
		except Exception as exc:
			ctx.set_error(exc)
			ctx.current_stats_snapshot = None
			return []

		ctx.current_stats_snapshot = snapshot
		if ctx.selected_stats_page_index >= snapshot.total_pages:
